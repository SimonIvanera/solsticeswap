  ...
  useEffect(() => {
    const loadPersistedState = async () => {
      try {
        ...
        const connected = await storage.getItem("wallet.connected");

        if (connected === "true" && lastAccounts && lastChainId) {
          // Try to find provider by connector ID
          const providerDetail = providers.find(
            (p) => p.info.uuid === lastConnectorId
          );
          if (providerDetail) {
            // Silently reconnect using eth_accounts
            try {
              ...
              if (accountsArray && Array.isArray(accountsArray) && accountsArray.length > 0) {
                ...
                // Clear persisted state if no accounts
        ...
        console.error("Failed to load persisted wallet state", e);
      }
    };
  ...
  useEffect(() => {
    ...
    if (next) {
      ...
      nextChainChangedListener = (chainId: string) => {
        ...
        storage.setItem("wallet.lastChainId", chainId);
      };
      ...
      const updateChainId = async () => {
        ...
        try {
          ...
          if (accountsArray && Array.isArray(accountsArray) && accountsArray.length > 0) {
            ...
            if (providerDetail) {
              await storage.setItem("wallet.lastConnectorId", providerDetail.info.uuid);
            }
            await storage.setItem("wallet.lastAccounts", JSON.stringify(accountsArray));
            await storage.setItem("wallet.lastChainId", chainIdHex as string);
